...
public enum LocalSnapSlot { Free, Full, Left, Right, TopLeft, TopRight, BottomLeft, BottomRight }
...
public readonly record struct PixelRect(int X, int Y, int Width, int Height)
{
    ...
    public long Overlap(PixelRect b) =>
        (long)Math.Max(0, Math.Min(Right, b.Right) - Math.Max(X, b.X)) *
        Math.Max(0, Math.Min(Bottom, b.Bottom) - Math.Max(Y, b.Y));

    public PixelRect FitInside(PixelRect b)
    {
        int w = Math.Clamp(Width, 1, b.Width), h = Math.Clamp(Height, 1, b.Height);
        return new(Math.Clamp(X, b.X, b.Right - w), Math.Clamp(Y, b.Y, b.Bottom - h), w, h);
    }
}
...
public sealed class AppConfig
{
    ...
    public static Dictionary<string, string> DefaultHotkeys()
    {
        var h = new Dictionary<string, string>
        {
            ...
            ["up"] = "Ctrl+Alt+Up",
            ["down"] = "Ctrl+Alt+Down",
            ["layout:single"] = "Ctrl+Alt+Shift+1",
            ["layout:dual-vertical"] = "Ctrl+Alt+Shift+2",
            ["layout:triple-main"] = "Ctrl+Alt+Shift+3",
            ["layout:quad"] = "Ctrl+Alt+Shift+4",
            ["layout:six"] = "Ctrl+Alt+Shift+6",
...
public sealed class WorkspaceWindow
{
    ...
    public LocalSnapSlot SnapSlot { get; set; }
}
